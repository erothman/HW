...
/*
 *This function prints out the encrypted input text using the vigenere cipher and the key given.
 */
func encryptMessage(key, input string) string {
	output := ""
	keyPlacer := 0
	for a := 0; a < len(input); a++ {
		letter := convertToAlph(input[a])
		if (letter != 100) {
			newLetter := (((letter + convertToAlph(key[keyPlacer])) % 26) + 65)
